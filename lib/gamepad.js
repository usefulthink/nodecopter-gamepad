...
exports.init = function(arDroneClient, controlEventEmitter) {
    ...
    arDroneClient.on('navdata', function(navdata) {
        isFlying = navdata.droneState.flying;
    });
    ...
    controlEventEmitter.on('control', function(ev) {
        ...
        if(isFlying === undefined) {
            console.error('undefined flying-state, not sending command');

            return;
        }

        if(actionClass == 'action' && action !== 'animate') {
            if(action == 'takeoffOrLand') {
                if(isFlying === undefined) {
                    console.error('takeoffOrLand called without known flyingState');
                } else {
                    action = (isFlying === true)? 'land' : 'takeoff';
                }
            }
